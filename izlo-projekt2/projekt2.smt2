...
; x = A*B*2 
(define-fun x_f ((A Int) (B Int)) Int 
  (* 2 (* A B))
)
; y = (x < E) ?  x + 5*B : x - C
(define-fun y_f ((B Int) (C Int) (x Int) (E Int)) Int (
  ite (< x E) (+ x (* 5 B)) (- x C)
))
; z = (y+2 < D) ?  x*A - y*B : x*B + y*A
(define-fun z_f ( (A Int) (B Int) (D Int) (x Int) (y Int) ) Int (
  ite (< (+ y 2) D) (- (* x A) (* y B)) (+ (* x B) (* y A))
))
...
(assert (= x (x_f A B)))
(assert (= y (y_f B C x E)))
(assert (= z (z_f A B D x y)))
...
(assert (
  not (
    exists ((d Int) (e Int) (yy Int) (zz Int)) (
      and
      (< (+ d e) (+ D E))
      (> d 0)
      (> e 0)
